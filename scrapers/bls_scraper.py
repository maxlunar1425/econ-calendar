...
def fetch_ics(url: str) -> str:
    """Uses curl_cffi (impersonating Chrome's TLS fingerprint) to fetch
    the BLS feed. This is REQUIRED, not optional -- BLS's site blocks
    requests with a plain urllib TLS signature with an HTTP 403, even
    with a realistic User-Agent header. This isn't an IP-based block
    (confirmed: it happened both from GitHub Actions AND from a normal
    home network) -- it's fingerprinting the TLS handshake itself, which
    only a library actually mimicking a real browser's TLS stack (like
    curl_cffi) gets past. A plain-urllib fallback is kept below for
    visibility if curl_cffi ever becomes unavailable, but it will almost
    certainly 403 -- see workflow requirements: `pip install curl_cffi`
    ...
    try:
        ...
        print(
            "  [!] curl_cffi not installed -- falling back to urllib, which "
            "will likely hit BLS's TLS-fingerprint block. Add "
            "'pip install curl_cffi' as a workflow step.",
            file=sys.stderr,
        )
    except Exception as e:
        print(f"  [!] curl_cffi fetch failed ({e}) -- falling back to urllib", file=sys.stderr)
