...
| `dol_scraper.py` | Weekly Jobless Claims — generates "every Thursday" + applies DOL's official holiday exceptions (recurring-rule pattern, not a fixed date list) |
| `build_data_releases_json.py` | Runs BLS + BEA + DOL, writes `sources/data_releases.json` |
| `watched_releases.json` | **The one file you'll actually edit.** Controls which BLS/BEA releases are included. (DOL has no watchlist — only one release type exists, always included.) |
...
**US data releases** (via `watched_releases.json` for BLS/BEA, always-on
for DOL): CPI, PPI, ECI, JOLTS, U.S. Import and Export Price Indexes, PCE
(via BEA's "Personal Income and Outlays"), and weekly Jobless Claims (DOL).

**Jobless Claims is structurally different from everything else** — DOL
doesn't publish a list of dates, just a rule ("every Thursday, 8:30am ET")
plus a short official exceptions table for federal holidays. See
`scrapers/dol_scraper.py`'s docstring for how the recurring-rule +
exception-override logic works.
...
2. **A scraper starts failing** (bank redesigns its page) → check the
   ...
   before you fix it.
3. **DOL shows a wrong Thursday around a federal holiday** → not a bug,
   just DOL not having published that year's exception yet. Nothing to
   fix on your end; it self-corrects once DOL posts it and the next
   weekly scrape runs.
4. **Add a new central bank or agency** → same pattern each time: (a)
   ...
   `_source.py` loader's title/URL mapping, (e) if it recurs more than
   once a month, remember the `uid_granularity: "day"` fix from the
   caveats below.
...
- **DOL's holiday exceptions list is sparse and not published far ahead**
  — if a future federal holiday shifts a Thursday release and DOL hasn't
  posted the exception yet, the calendar will show a plain (wrong)
  Thursday until DOL publishes it and the next weekly scrape picks it up.
- **UID granularity matters and has already caused one real bug.** Events
  recurring monthly-or-less (central banks, BLS, BEA) use month-level
  UIDs so a small date correction updates in place instead of
  duplicating. DOL recurs weekly, so it needed day-level UIDs (see
  `uid_granularity` in `ics_utils.py` / `data_release_source.py`) —
  without this, multiple real weekly events in the same month silently
  collapsed into one. If a future addition also recurs more than once a
  month, it needs this same `"uid_granularity": "day"` treatment.
